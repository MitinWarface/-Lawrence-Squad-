...
const FALLBACK_CHANNEL_NAME = 'Голосовая комната';
...
export default {
  ...
  async execute(oldState, newState, client) {
    ...
    try {
      ...
      logger.error(`Ошибка в voiceStateUpdate для сервера ${guildId}:`, error);
    }
    ...
    async function handleVoiceJoin(client, state, config) {
      ...
      if (channelCreationCooldown.has(cooldownKey)) {
        ...
        if (now - lastCreation < VOICE_CREATE_COOLDOWN_MS) {
          logger.warn(`Пользователь ${member.id} находится на перезарядке создания канала`);
          return;
        }
      }
      ...
      if (existingTempChannel) {
        ...
        if (tempChannel) {
          try {
            ...
            logger.warn(`Не удалось переместить пользователя ${member.id} в существующий канал ${existingTempChannel}:`, error);
          }
        }
      }
    ...
    async function createTemporaryChannel(client, state, config) {
      ...
      try {
        ...
        if (!me) {
          logger.warn(`Кэш участника-бота недоступен при создании временного канала на сервере ${guild.id}`);
        ...
        if (!triggerPermissions?.has([PermissionFlagsBits.ManageChannels, PermissionFlagsBits.MoveMembers, PermissionFlagsBits.Connect])) {
          logger.warn(`Недостаточно прав для создания временного канала на сервере ${guild.id} (триггер-канал ${triggerChannel.id})`);
          channelCreationCooldown.delete(cooldownKey);
          return;
        }

        const channelOptions = config.channelOptions?.[triggerChannel.id] || {};
        const nameTemplate = channelOptions.nameTemplate || config.channelNameTemplate || "Комната {username}";
        ...
        logger.info(`Создание временного канала для пользователя ${member.id} с лимитом пользователей: ${userLimit}`);
        ...
        if (!member.voice?.channel || member.voice.channel.id !== triggerChannel.id) {
          logger.debug(`Участник ${member.id} больше не находится в триггер-канале ${triggerChannel.id}, прерывание создания временного канала`);
          channelCreationCooldown.delete(cooldownKey);
          return;
        }
        ...
        if (member.voice?.channel?.id === triggerChannel.id) {
          ...
          logger.debug(`Пропущен перенос ${member.id} во временный канал ${tempChannel.id}, так как голосовой статус изменился`);
        }

        logger.info(`Создан временный голосовой канал ${tempChannel.name} (${tempChannel.id}) для пользователя ${member.user.tag} на сервере ${guild.name} с лимитом пользователей ${userLimit}`);

      } catch (error) {
        logger.error(`Не удалось создать временный канал для пользователя ${member.user.tag} на сервере ${guild.name}:`, error);
        
        channelCreationCooldown.delete(cooldownKey);
        
        try {
          await member.send({
            content: `❌ Не удалось создать ваш временный голосовой канал. Пожалуйста, свяжитесь с администратором сервера.`
          });
        } catch (dmError) {
          logger.debug(`Невозможно отправить личное сообщение об ошибке создания временного канала пользователю ${member.id}:`, dmError);
        }
      }
    }
    ...
    async function deleteTemporaryChannel(client, channel, guildId) {
      try {
        ...
        await channel.delete('Временный голосовой канал - пуст');

        logger.info(`Удален временный голосовой канал ${channel.name} (${channel.id}) на сервере ${channel.guild.name}`);

      } catch (error) {
        logger.error(`Не удалось удалить временный канал ${channel.id}:`, error);
      }
    }
      ...
      try {
        ...
        if (newOwner) {
          ...
          const newChannelName = sanitizeVoiceChannelName(formatChannelName(nameTemplate, {
            ...
            channelName: channel.guild.channels.cache.get(tempChannelInfo.triggerChannelId)?.name || 'Голосовой канал'
          }));

          await channel.setName(newChannelName);
        }

        logger.info(`Права владения временным каналом ${channel.id} переданы пользователю ${newOwnerId}`);

      } catch (error) {
        logger.error(`Не удалось передать права владения каналом ${channel.id}:`, error);
      }
    }
  }
};
